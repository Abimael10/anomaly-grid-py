...
class TestProbabilityTheory:
    ...
    def test_2_1_kolmogorov_probability_axioms(self):
        ...
        for context in contexts:
            ...
            assert all(p >= 0 for p in context_probs), f"Negative probabilities for context {context}"
    ...
    def test_2_2_conditional_probability_rules(self):
        ...
        for context, next_state in test_pairs:
            ...
            print(f"      P({context},{next_state}) = {joint_prob:.6f}, P({next_state}|{context}) = {conditional_prob:.6f}")
            print(f"      P({next_state}|{context}) = {conditional_prob:.6f}, P({context}|{next_state}) = {context_prob:.6f}, difference = {difference:.6f}")
    ...
    def test_2_3_bayes_theorem(self):
        ...
        for event_a, event_b in events:
            ...
            print(f"    Expected P({event_a}|{event_b}) via Bayes = {expected_p_a_given_b:.6f}")
    ...
    def test_2_4_law_of_total_probability(self):
        ...
        for partition_event in partition:
            ...
            print(f"    P(X|{partition_event}) * P({partition_event}) = {p_x_given_partition:.6f} * {p_partition:.6f} = {contribution:.6f}")
    ...
    def test_2_5_independence_and_correlation(self):
        ...
        for event_a, event_b in independent_tests:
            ...
            print(f"    P({event_a}) * P({event_b}) = {p_a:.6f} * {p_b:.6f} = {expected_if_independent:.6f}")
    ...
    def test_2_6_probability_bounds_and_inequalities(self):
        ...
        for i, score in enumerate(scores):
            ...
            assert -1.0 <= prob <= 2.0, f"Derived probability out of reasonable bounds: {prob}"
