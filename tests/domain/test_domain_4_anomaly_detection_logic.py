...
class TestAnomalyDetectionLogic:
    ...
    def test_4_1_anomaly_definition_consistency(self):
        ...
        for i, score in enumerate(normal_scores):
            ...
            assert score < 0.2, f"Normal sequence {i} has high anomaly score: {score:.6f}"
        ...
        for threshold in thresholds:
            ...
            assert 0 <= anomaly_count <= len(all_test_sequences), f"Invalid anomaly count: {anomaly_count}"
    ...
    def test_4_3_threshold_sensitivity_analysis(self):
        ...
        true_labels = [0] * len(self.normal_test_sequences) + [1] * len(self.anomalous_test_sequences)
        ...
        for threshold in thresholds:
            ...
            print(f"    {threshold:9.2f} | {detected_count:8d} | {precision:9.3f} | {recall:6.3f} | {f1:8.3f}")
            
            # Verify monotonicity: higher thresholds should detect fewer or equal anomalies
            assert detected_count <= previous_detected, f"Threshold monotonicity violation at {threshold}"
    ...
    def test_4_4_roc_auc_performance(self):
        ...
        try:
            ...
            if auc < 0.6:
                print(f"    ⚠️  Warning: Low AUC score ({auc:.3f}) - detector may need tuning")
            ...
            assert abs(anomalous_avg - normal_avg) > 0.01, "No discrimination between normal and anomalous"
    ...
    def test_4_5_sequence_length_robustness(self):
        ...
        length_tests = [
            ...
            # Long sequences
            (['LOGIN', 'BALANCE', 'TRANSFER', 'WITHDRAW', 'BALANCE', 'LOGOUT'], "Long normal"),
            (['HACK', 'EXPLOIT', 'ESCALATE', 'PERSIST', 'EXFILTRATE', 'ESCAPE'], "Long anomalous"),
            
            # Very long sequences
            (['LOGIN'] + ['BALANCE', 'WITHDRAW'] * 5 + ['LOGOUT'], "Very long normal"),
            (['ATTACK'] + ['EXPLOIT', 'PERSIST'] * 5 + ['ESCAPE'], "Very long anomalous")
        ]
        ...
        for sequence, description in length_tests:
            ...
            print(f"      {description:20s} (len={len(sequence):2d}): score = {score:.6f}")
            
            # Verify score is reasonable
            assert 0.0 <= score <= 1.0, f"Score out of bounds for {description}: {score}"
    ...
    def test_4_6_consistency_and_reproducibility(self):
        ...
        assert score_variance < 1e-10, f"Inconsistent scores: variance = {score_variance}"
        ...
        assert abs(equiv_scores[0] - equiv_scores[1]) < 1e-10, "Identical sequences have different scores"
