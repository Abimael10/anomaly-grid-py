...
class TestMarkovChainMathematics:
    ...
    def test_1_1_markov_property_memoryless(self):
        ...
        assert abs(scores[0] - scores[3]) < 0.001, "Same sequences should have identical scores"
        ...
        if markov_violation >= markov_tolerance:
            ...
            print(f"    This is expected with higher max_order values for practical anomaly detection")
        else:
            print(f"    ✅ Markov property satisfied: violation {markov_violation:.6f} < {markov_tolerance}")
    ...
    def test_1_3_chapman_kolmogorov_equation(self):
        ...
        print(f"    Sequence: A -> B")
        print(f"    P(B|A) = {1.0 - scores[0]:.6f}")
        print(f"    Sequence: B -> C")
        print(f"    P(C|B) = {1.0 - scores[1]:.6f}")
        print(f"    Sequence: A -> B -> C")
        ...
        assert ratio < tolerance, f"Chapman-Kolmogorov violation: ratio {ratio:.3f} > {tolerance}"
    ...
    def test_1_4_stationarity_and_time_homogeneity(self):
        ...
        assert variation < 0.8, f"Too much variation in transition probabilities: {variation:.3f}"
    ...
    def test_1_5_ergodicity_and_convergence(self):
        ...
        for i, score in enumerate(scores):
            assert score < 0.1, f"Long trained pattern {i+1} has high anomaly score: {score:.6f}"
    ...
    def test_1_6_mathematical_consistency(self):
        ...
        assert scores[1] < scores[2], "Exact training pattern should have lower anomaly score than unseen pattern"
        assert scores[2] > scores[0], "Unseen pattern should have higher anomaly score than repeated element"
