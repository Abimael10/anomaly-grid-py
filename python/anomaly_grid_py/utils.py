...
def train_test_split(sequences: List[List[str]], 
                    test_size: float = 0.2, 
                    random_state: int = None) -> Tuple[List[List[str]], List[List[str]]]:
    """
...
def cross_val_score(estimator, X: List[List[str]], y: List[int], 
                   cv: int = 3) -> np.ndarray:
    """
...
def generate_sequences(n_sequences: int, 
                      seq_length: int, 
                      alphabet: List[str],
                      anomaly_rate: float = 0.1) -> Tuple[List[List[str]], List[int]]:
    """
...
def validate_sequences(sequences: List[List[str]], min_length: int = 2) -> None:
    ...
    for i, sequence in enumerate(sequences):
        ...
        if len(sequence) < min_length:
            raise ValueError(f"Sequence {i} has length {len(sequence)}, minimum is {min_length}")
        
        for j, element in enumerate(sequence):
            if not isinstance(element, str):
                raise TypeError(f"Element at sequence {i}, position {j} must be a string")
